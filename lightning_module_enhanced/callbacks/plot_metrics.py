...
class PlotMetrics(pl.Callback):
    ...
    def _do_plot(self, pl_module: Any, df: dict, metric_name: str, out_file: str):
        ...
        ax.plot(x_plot, _norm((train_y := df[metric_name].tolist()), metric_name), label="train")
        if (val_y := df.get(f"val_{metric_name}")) is not None:
            ax.plot(x_plot, _norm(val_y.tolist(), metric_name), label="validation")
        self._plot_best_dot(ax, train_y if val_y is None else val_y.tolist(), higher_is_better)
        ax.set_xlabel("Epoch")
    ...
    def on_train_epoch_start(self, trainer: pl.Trainer, pl_module: Any):
        if not Path(f"{self.log_dir}/metrics.csv").exists():
            ...
            return
        df = pd.read_csv(f"{self.log_dir}/metrics.csv")
        expected_metrics: list[str] = [*list(pl_module.metrics.keys()), "loss"]
        for metric_name in expected_metrics:
            if metric_name not in df:
                logger.debug(f"'{metric_name}' not in {list(df.columns)}")
                continue
            self._do_plot(pl_module, df, metric_name, out_file=f"{self.log_dir}/{metric_name}.png")
