...
def test_r_metrics_grad():
    module = LightningModuleEnhanced(nn.Sequential(nn.Linear(10, 2), nn.Linear(2, 3)))
    module.criterion_fn = lambda y, gt: (y - gt).abs().mean()
    module.metrics = {
        "metric1": CallableCoreMetric(metric_grad, requires_grad=True),
        "metric2": metric_non_grad
    }
